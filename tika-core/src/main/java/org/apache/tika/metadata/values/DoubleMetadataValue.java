...
public class DoubleMetadataValue extends MetadataValue {
    ...
    public DoubleMetadataValue(final String val) {
        super(val);
        Double tmp = null;
        try {
            tmp = Double.valueOf(val);
        } catch(NumberFormatException e) {
        }
        d = tmp;
        resetStringVal((d == null) ? val : Double.toString(d));
    }

    public DoubleMetadataValue(Double d) {
